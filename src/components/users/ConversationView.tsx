    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-white/5 px-1 pb-3">
        <div className="flex items-center gap-2 text-xs">
          {hasTelegram && (
            <span className="rounded-md bg-white/5 px-2.5 py-0.5 font-semibold text-slate-300 ring-1 ring-white/10">Telegram</span>
          )}
          {hasMobile && (
            <span className="rounded-md bg-sky-500/10 px-2.5 py-0.5 font-semibold text-sky-300 ring-1 ring-sky-500/20">Приложение</span>
          )}
            ...
            return (
              <div key={m.id} className={`flex w-fit max-w-[80%] flex-col ${wrapAlign}`}>
                <div className={`rounded-md px-4 py-3 text-xs leading-relaxed ${bubbleCls}`}>
                  <div className="mb-1.5 flex items-center gap-2 text-[10px] font-semibold tracking-wide uppercase opacity-75">
                    <span>{m.role}</span>
